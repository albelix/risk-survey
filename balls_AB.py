...
Wiki
Insights
guzey.github.io1 / balls / index.html
7461e46
on
Feb
14
...
< p > < i > Source
code is < a
href = "https://github.com/guzey/guzey.github.io/blob/master/balls/index.html" > here < / a >.Game
demo
directly
below
text.I
...
the
code
for the project, while Grigory Chernov (PhD student at Higher School of Economics), Alexis Belianin (Assistant Professor of Economics at Higher School of Economics), and me jointly discussed the ideas.< / i > < / p >
< p > People
have
different
risk
...
Elicitation
Task(BRET),
with BART being the older one, and being gradually supplanted by BRET.However, in our view, both of them are inadequate: in
both
games
it
's initially unclear to the player how exactly is risk related to the expected profit, thus we cannot distinguish whether the person is seeking risk, trying to avoid it, or is simply trying to maximize their profit (risk-neutral).</p>
< p > The
end
goal
of
...
BART and BRET
measure
risk - preferences in a
setting, where
the
choice
of
the
strategy is "a priori" and not based
on
the
skill
level.In
...
widespread, yet
they
're impossible to measure today. Examples of such cases are: choice of driving speed in a car, choice of the level of course load in a university, choice of the computer game difficulty, etc.</p>
< p > The
design
was
guided
...
make
decisions, which
results in moderate, but
not high
enough, strategy
variability, and high
noise. < / li >
< li > Just
like in BART and BRET
we
are
still
unable
...
profit
maximization
strategies or anything else.< / li >
< li > The
fake
balls
were
...
load).This
doesn
't seem to be the case for any design we tried. People adapt to ignoring fake balls very quickly, not really bearing any cognitive load.</li>
< li > Lastly, we
underestimated
the
technical
...
dynamic.In
particular, it
's not clear if this design can be adapted so that the game could be played on a phone (or even simply without a computer mouse) and herewith save its core mechanic.</li>
< / ul >
< p > The
problem
of
...
name = "current_round"
value = "0"
id = "hidden_current_round" / >
< input
type = "hidden"
name = "current_batch"
value = "0"
id = "hidden_current_batch" / >
< input
type = "hidden"
name = "probability_out"
value = "0"
id = "hidden_probability_out" / >
< input
type = "hidden"
name = "sigmoid_probability_out"
value = "0"
id = "hidden_sigmoid_probability_out" / >
< !--records
game
score -->
< input
...
class ="slidecontainer" id="slidecontainer" style="position: absolute; margin-top: -260px; margin-left: 140px; z-index: -1;" >
...
id = "slider" >
< p
style = "font-size: 16px;" > Number
of
fake
balls: < span
id = "slider_output" > < / span > < / p >
< p
style = "font-size: 16px;" > Score
multiplicator: < span
id = "slider_output_multiplicator" > < / span > < / p >
< / div >
< script >
let
...
's
// let
rewards = [[10, 20, 30, -30], [10, 50, 100, -100], [50, 100, 150, -150], [10, 50, 150, -150],
          // [50, 100, 100, -100], [10, 20, 30, -200], [10, 50, 100, -200]],
// shortened
// let
rewards = [[10, 20, 30, -30], [20, 40, 60, -60]],
num_rounds = rewards.length
let
center_reward = rewards[0][0],
...
{
    next_button_text = new
Text("Next", {font: "64px Futura", fill: "blue"}
     )
...
{
// ball
's direction angle in radians; min angle between two successive balls is pi/2
// do
{
...
{
let
mouseData = renderer.plugins.interaction.mouse.
global
current_balls -= 1
...
cognitive_load_text = new
Text("Please select the cognitive load level\n\n" +
...
//
start_round_button_rectangle = new
Graphics()
...
start_round_button_text = new
Text("Start the next round", {font: "64px Futura", fill: "blue"}
     )
...
// doesn
't do anything specific--simply runs the game
function
appLoop()
{
// Loop
...
for_hidden_current_round = [],
                           for_hidden_current_batch = [],
                                                      for_hidden_num_clicks = new
Array(num_rounds * round_duration * 3).fill(0),
clicks = 0,
         for_hidden_ball_out = [],
...
final_multiplicator = roundToTwo(1 + point_multiplicator * slider.value)
                      // new
condition:
if you don't tap all 3 balls in the same zone, you get 0
